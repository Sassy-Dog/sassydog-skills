...
name: drain-it
description: >
  Loop-driven dispatcher for ai-agent-skills: each invocation is one idempotent tick that
  reconciles in-flight PRs, then tops the pipeline back up to 3 concurrent
  issues pulled ONLY from open issues carrying the `ready` label, respecting dependencies and
...
One invocation = **one tick** of a drain loop. All state lives in GitHub (labels, assignees, PRs, branches) — never in conversation memory, because under `/loop` each tick may run with no recollection of the previous one. The contract with fill-it: **drain-it pulls exclusively from Ready** (the `ready` label) and trusts that Ready means dispatchable; anything that smells undispatchable gets bounced back, never patched up inline.
...
| Filter | Rule |
|--------|------|
| Claimed | Skip if assignee set or `in-progress` label present (another session got it) |
...
Zero open issues with the `ready` label AND in-flight zero, **confirmed from live GitHub state read this tick** (the §1 reconcile + §3 query — never a stale or transient read) → announce loudly:
...
Safety rails: self-cancel ONLY on the confirmed complete state above. Anything still claimed (assignee @me + `in-progress`), an open PR (in-flight until actually MERGED, per §2), or any open issue still labeled `ready` means the drain is not complete and the loop stays alive. If live state could not be verified this tick (e.g. API failure mid-tick), leave the cron alone — the next tick re-checks. Ticks that fire between completion and cancellation are no-ops, not errors: each one re-runs this section and retries the self-cancel.

## Guardrails

- **Ready only.** Issues not labeled `ready` are fill-it's job — drain-it never promotes, never grooms, never files issues.
