...
Each brief contains, and contains only — the delivery rule at 6 included, since a list that omits it cannot pass the contract down ([#280](https://github.com/Sassy-Dog/skills/issues/280)):
...
5. The instruction to return **only** the JSON object `{"findings": [...]}` in its own documented schema, `{"findings": []}` if it finds nothing. With no assessment rubric in play, `area` carries the surface name it was dispatched for.
6. **How the findings come back.** State it: its object envelope is its **returned final text** — the last thing it says, and nothing else, with no Markdown fences or surrounding prose. A message is not a delivery mechanism for findings, a file it wrote is not one either, and `{"findings": []}` is *returned* rather than left unsaid. Each of the nine carries this rule in its own file; say it in the brief anyway, because a contract only one end holds is one a dispatch silently drops — which is what the bullet that scores a lost reviewer keeps catching.
