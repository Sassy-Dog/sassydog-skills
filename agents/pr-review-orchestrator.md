...
- **Known limit — this rule binds YOUR delivery, not your reviewers'** ([#280](https://github.com/Sassy-Dog/sassydog-skills/issues/280)). The nine agents you fan out to carry no delivery rule of their own, and the brief above is a closed list with no slot to put one in, so you cannot pass this contract down by following it. That is why the bullet above it is the backstop and not a formality: a reviewer that did not come back is scored `!` and named, on every run. Do not read a clean fan-out as proof the hop worked — read the `!` count. #273's own review ran degraded on exactly this, losing three of four surfaces.
