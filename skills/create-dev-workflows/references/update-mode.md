...
Generated skills use plain names (`plate-it`, `take-it`, `send-it`). Before writing, check for shadowing collisions: a **personal** skill (`~/.claude/skills/<same-name>`) hard-shadows a project skill of the same name. If one exists, STOP and tell the user to remove/rename it — the generated skill would be invisible.
