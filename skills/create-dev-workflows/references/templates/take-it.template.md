...
**First, fast-forward local {{DEFAULT_BRANCH}}** — worktrees branch from local HEAD, not origin; a stale base lands the PR `CONFLICTING`:

```bash
git fetch origin {{DEFAULT_BRANCH}} --quiet
git switch {{DEFAULT_BRANCH}} >/dev/null 2>&1 && git pull --ff-only origin {{DEFAULT_BRANCH}}
```
...
> You are shipping GitHub issue **#{N}** in the {{PROJECT_NAME}} repo ({{STACK_SUMMARY}}).
>
...
> 7. Push and open a PR per the send-it template — the body MUST contain `Closes #{N}` on its own line ({{PR_TEMPLATE_SECTIONS_SUMMARY}}).
