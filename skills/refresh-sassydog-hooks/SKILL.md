...
of it on every re-run. The sibling of `refresh-sassydog-skills` — same philosophy: the plugin ships
...
- Generated scripts are exactly the files matching `.claude/hooks/sassydog-*.sh`, each carrying a
  `generated-by: ai-agent-skills:refresh-sassydog-hooks` header comment.
...
```bash
bash ${CLAUDE_PLUGIN_ROOT}/skills/refresh-sassydog-hooks/scripts/detect-hook-stack.sh
```
