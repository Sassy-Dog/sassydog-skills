...
- **Lagging AND unmanaged** → the fix is to render the config (`ai-agent-skills:refresh-sassydog-deps`),
...
`refresh-sassydog-deps`) acts. Keeping the audit read-only is what makes it safe to run on a
