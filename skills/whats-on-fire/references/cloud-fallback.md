...
| `default_branch_ci_age_days`, `default_branch_ci_url` | Age in days of the run the verdict came from, and its link — projected from the SAME run, never from a different one. The recovery is not bounded by the sample size, so the verdict can be weeks old; `scoring.md` bounds both directions at 14 days and cannot apply that rule without these. `last_failure` is derived from the sample, so a RECOVERED failure has none beside it and this url is its only link. |
