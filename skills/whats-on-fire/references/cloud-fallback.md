...
| `default_branch_ci` | Conclusion of the MOST RECENT completed run on the default branch whose event is `push` or `merge_group`. Never widen to `schedule` — the push/schedule split is load-bearing (section 2B). **Null when the sample holds no such run** — that is unknown, never clean and never P0 (`scoring.md`). |
