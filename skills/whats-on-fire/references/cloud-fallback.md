...
| `default_branch_ci` | Conclusion of the MOST RECENT completed run on the default branch whose event is `push` or `merge_group`. Never widen to `schedule` — the push/schedule split is load-bearing (section 2B). **Null when the sample holds no such run** — that is unknown, never clean and never P0 (`scoring.md`). Recover it before reporting: re-issue the runs call for that repo narrowed to `{branch: <default_branch>, event: push}` and reduce the result the same way. `event` is the half that does the work — a busy default branch is crowded by `schedule` runs, which a branch filter keeps. |
| `default_branch_runs_seen` | How many runs were push-class AND on the default branch, counted **regardless of status**, over whichever query answered — the sample, or the recovery re-query when one was needed. This is what makes a surviving null readable: `0` means no such run was there to read, non-zero means they are all still in flight. Report the two differently (`scoring.md`). |
