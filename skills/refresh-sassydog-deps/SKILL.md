...
`refresh-sassydog-skills` and `refresh-sassydog-hooks`: detect the stack, render from templates,
...
Ownership marker: the `generated-by: ai-agent-skills:refresh-sassydog-deps` comment on the first
...
```bash
bash ${CLAUDE_PLUGIN_ROOT}/skills/refresh-sassydog-deps/scripts/detect-ecosystems.sh
```
