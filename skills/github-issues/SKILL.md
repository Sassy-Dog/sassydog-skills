...
One call replaces the per-tick ready/in-flight/blocked list reads AND parses the four machine-readable body contracts — `touches:` collision sets, literal `Depends on #N` lines, `stack:` chains, and the `site:` execution site — in one place:

```bash
REPO=<owner/name> bash ${CLAUDE_PLUGIN_ROOT}/skills/github-issues/scripts/queue-snapshot.sh
```

Emits `{repo, me, ready[], in_flight[], blocked[]}` — `ready` ordered number-ascending with `touches`/`stack`/`site`/`depends_on`/`unannotated` per issue, `in_flight` with the same set minus `depends_on`/`unannotated` plus a `mine` flag, carried rather than silently filtering to @me so a loop can count its own claims while still seeing other sessions'. `site` is the issue's declared execution site or `null` for "any site" ([#340](https://github.com/Sassy-Dog/sassydog-skills/issues/340)); the script's header states how a body with several or a malformed one resolves. Judgment (touches-set intersection, priority, site matching, the smell test) stays with the caller — this is a read, not a dispatcher.
...
| `scripts/queue-snapshot.sh` | Boardless fill/drain queue read: ready/in-flight/blocked buckets + the parsed `touches:`, `Depends on #N`, `stack:` and `site:` body contracts. `site:` alone is masked inside a fenced block, and reads the comment-stripped **remainder** of its line, so a body may quote the contract and an unfilled `site: <!-- vdi \| mac -->` template placeholder declares nothing rather than declaring `<!--`. Fence delimiters pair on character, run length and a whitespace-only tail, so a quoted example cannot end the block quoting it. The token is bounded by a grammar (`^[a-z0-9][a-z0-9._-]{0,63}$` after folding) so a consumer is never handed shell metacharacters or an unbounded string; nothing inside it is reserved, so `site: any` is a site called `any` and it is the absent line that means "any site". The older three keep the fence-blind parse they shipped with. Read-only. Exit 10 skip convention. |
