...
Output actions: `filed` / `already-linked` / `filed-no-board` / `would-file`. An `already-linked` carries `via: search | recent-scan` naming which idempotency stage answered. Board placement is optional and degrades gracefully. Exit `2` includes "the idempotency scan could not be performed" — nothing was filed, and retrying is safe.
