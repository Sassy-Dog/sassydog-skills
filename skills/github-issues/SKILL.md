...
| `scripts/queue-snapshot.sh` | Boardless fill/drain queue read: ready/in-flight/blocked buckets + the parsed `touches:`, `Depends on #N`, `stack:` and `site:` body contracts. `site:` alone ignores lines inside a fenced block or an HTML comment, so a body quoting the contract does not declare one; the older three keep the fence-blind parse they shipped with. Read-only. Exit 10 skip convention. |
