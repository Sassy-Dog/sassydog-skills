...
Emits `{repo, me, ready[], in_flight[], blocked[]}` — `ready` ordered number-ascending with `touches`/`stack`/`site`/`sites`/`depends_on`/`unannotated` per issue, `in_flight` with the same set minus `depends_on`/`unannotated` plus a `mine` flag, carried rather than silently filtering to @me so a loop can count its own claims while still seeing other sessions'. The **execution site comes from a `site:<name>` LABEL**, never from the body ([#340](https://github.com/Sassy-Dog/sassydog-skills/issues/340)) — a body line can be quoted in prose, a label cannot. `sites` is the sorted set of declared values and is the fact; `site` is the scalar and is set only when exactly one site is declared, so **a null `site` beside a non-empty `sites` is a conflict, not an absence** — read `sites`. Judgment (touches-set intersection, priority, site matching, the smell test) stays with the caller — this is a read, not a dispatcher.
...
| `scripts/queue-snapshot.sh` | Boardless fill/drain queue read: ready/in-flight/blocked buckets + the parsed `touches:`, `Depends on #N` and `stack:` body contracts, plus the execution site read from `site:<name>` **labels** (prefix-matched, folded, empty value ignored). `sites` is the sorted set and the fact; `site` is set only when exactly one is declared, so a conflict reads as null-with-a-non-empty-`sites` and never as "any site". Read-only. Exit 10 skip convention. |
