...
4. No existing GitHub peer: no `sentry-source: <SHORT_ID>` marker in any issue, open or closed. **A marker search here is a preview HINT, never a verdict — a hit must not drop the candidate.** `gh issue list --search '"sentry-source: <SHORT_ID>" in:body' --state all` matches a token *subsequence*, not the literal marker: `sentry-source: FOO` matches an existing `sentry-source: FOO-BAR`, so a real escalation is discarded before it reaches the filing step, reported as `already-linked` against somebody else's issue. Verified read-only on 2026-09-04 — `"stale-issues-title-only" in:body` returns issues whose marker is `stale-issues-title-only-shipped-detector`, while a superstring control returns `[]`. Only `file-or-link-issue.sh` decides this, because only it matches the delimited `<!-- <marker> -->` footer over bodies it fetches, across both its stages. Tag `already-linked` from **its** verdict (`--dry-run` reports `action` and `via`), never from a search run here.
