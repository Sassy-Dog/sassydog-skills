...
b009	**Report WHY an `unknown` could not measure, and treat a probe that says nothing as one.** The probe's `explains` field carries the reason behind an `unknown` and the first-party detail behind that — including its own `PLATFORM_GH_TIMEOUT` bound firing, which is the single most useful fact this loop can print during the outage the state exists for, and which the bare verdict flattens into the same sentence as "no `--pr` was given". So render the reason, not the word: `probe: unknown — not measured: pr_read_failed; gh pr view exited 124 — the 20s PLATFORM_GH_TIMEOUT bound fired`. A run that returns **no stdout at all** — killed by a tool timeout before it could emit, the one failure its own emitter fallback cannot answer because the script never reaches it — is `unknown` by the same rule and is reported as `probe: no verdict`. Neither form is `degraded`, neither stops the loop, and neither is `healthy`: a probe that did not speak is not a platform that is fine.
