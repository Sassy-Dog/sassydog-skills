...
#      openers, list markers, table rows and headings for §7 AND for §2, §6 and
#      the top-level Guardrails list — the three places outside §7 that can
#      override a §7 decision, and Guardrails already restates a §7-adjacent
#      rule today, so hoisting one there has precedent in that very file; (3)
...
#      reads as "not found", so thirteen vetoes were failing open. Every
...
#      hand; a gate cannot verify its own guard from inside it; and markdownlint
#      remains load-bearing for a malformed table. THE PREMISE IS ASSERTED, NOT
#      ASSUMED:
