...
- **`refresh-sassydog-skills` is a generator/refresher, not a runtime skill.** Its target state is writing each consumer repo's `.claude/sassy-dog/*.md` config plus the `.claude/settings.json` declaration. **It has not been converted yet** — it still renders the legacy per-repo skill bodies from `references/templates/`, so do not run it against a migrated repo until that conversion lands. This repo's own config under `.claude/sassy-dog/` was written by hand and is the reference target the converted refresher must reproduce.
- **Two delegation modes; vendored-by markers are the mode signal.** Plugin-backed renders delegate `Skill: ai-agent-skills:<cap>`; independent renders set `{{CAP_NS}}` empty (`Skill: <cap>`) + `IF:INDEPENDENT` and vendor the transitive capability bundle into the consumer repo's `.claude/skills/` with a `vendored-by:` marker per SKILL.md (contract: `refresh-sassydog-skills/references/independent-mode.md`). Mode persistence is repo state — any `vendored-by:` marker = independent; never store it as separate config. `pr-shepherd` is the mandatory bundle root (`repo-cleanup` ships no scripts and calls its `teardown.sh`; `github-issues` calls its `gh-retry.sh`). The refresher itself is never vendored — refresh always runs from the plugin.
- **`refresh-sassydog-hooks` is the same generator pattern one layer down**: it detects a consumer repo's stack (keyed on repo config presence, never installed binaries) and renders a single PostToolUse formatter/linter dispatcher into `.claude/hooks/sassydog-post-edit.sh` + a `.claude/settings.json` entry. Ownership marker = command path referencing `.claude/hooks/sassydog-`; re-runs reconcile only owned entries and never touch hand-written hooks. The template (`references/templates/sassydog-post-edit.template.sh`) is valid shellcheck-clean bash with ALL tool blocks present — rendering only deletes `# {{IF:TOOL}}` blocks, so every render is valid by construction. Like the skills refresher, it always runs from the plugin and is never vendored.
- **Generated files carry a `generated-by` header and `PROJECT-SPECIFIC` fences.** Update mode re-renders from current templates and splices the fences; adopt mode migrates legacy hand-written trios. The contract lives in `refresh-sassydog-skills/references/update-mode.md` — keep it in sync with the template headers. Recognition accepts both producer names (`refresh-sassydog-skills` current, legacy `create-dev-workflows` from plugin ≤ 0.8.1) so pre-rename consumer renders still update cleanly.
